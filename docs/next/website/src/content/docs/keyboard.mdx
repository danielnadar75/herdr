...
A prefix that combines a modifier with a printable key, such as `shift+space`, also needs `extended_keys = true` so the terminal reports it as a distinct key rather than a plain Space. See [extended keys](/docs/configuration/#keybindings).
