...
Some chords combine a modifier with a printable key, such as `shift+space` or `ctrl+shift+p`. Terminals send those as the bare character by default, so Herdr cannot tell `shift+space` apart from Space. Set `extended_keys` to ask the terminal to report every key as an escape code (the Kitty keyboard protocol's report-all-keys mode, equivalent to tmux's `set -s extended-keys on`):
...
It is off by default because report-all-keys can interfere with CJK IME composition. Herdr already withholds it while you are typing into its own text fields, such as the rename and search boxes. Your terminal must support the Kitty keyboard protocol; Ghostty, kitty, WezTerm, foot, and recent Alacritty do. When the client connects with `HERDR_REMOTE_KEYBINDINGS=local`, this setting travels with the rest of your local `[keys]` profile, so the remote session honors your local terminal's setting.
...
A tab you have not renamed already shows its jump number, because the number is its name. Rename it and the number is replaced. Turn on `show_tab_numbers` to keep the number visible on named tabs too, drawn in the theme accent color so it reads as a jump target for the `prefix+1..9` bindings:
...
`tab_number_start` accepts `0` or `1`; any other value falls back to `1` with a startup warning. Note that the numbers follow tab position, so they shift when you reorder or close tabs, and only `1` through `9` are reachable through the default indexed bindings.
